...
void generate_wifi_mac() {
  ...
  // force replace configuration file if vaild mac is in factory config file
  if (is_file_exists(WCND_WIFI_FACTORY_CONFIG_FILE_PATH)) {
    ...
    if (!is_zero_ether_addr(mac)) {
      force_replace_file(WCND_WIFI_CONFIG_FILE_PATH,
                         WCND_WIFI_FACTORY_CONFIG_FILE_PATH);
      return;
    }
  }
...
void generate_bt_mac() {
  ...
  if (is_file_exists(WCND_BT_FACTORY_CONFIG_FILE_PATH)) {
    ...
    if (!is_zero_ether_addr(mac)) {
      force_replace_file(WCND_BT_CONFIG_FILE_PATH,
                         WCND_BT_FACTORY_CONFIG_FILE_PATH);
