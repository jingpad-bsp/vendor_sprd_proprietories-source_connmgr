...
static int bcm_mkiovar(char *name, char *data, uint datalen, char *buf,
                       uint buflen) {
  uint len;
...
static int wl_ioctl(struct ifreq *ifr, int cmd, void *buf, int len, bool set) {
  wl_ioctl_t ioc;
  int ret = 0;
  int s;
...
static int connect_netd(void) {
  int client_fd = socket_local_client("netd", ANDROID_SOCKET_NAMESPACE_RESERVED,
                                      SOCK_STREAM);

  while (client_fd < 0) {
    WCND_LOGD("%s: Unable bind server netd, waiting...\n", __func__);
    usleep(100 * 1000);
    client_fd = socket_local_client("netd", ANDROID_SOCKET_NAMESPACE_RESERVED,
                                    SOCK_STREAM);
  }
...
static int wait_for_netd(int sock, int stop_after_cmd, char *resp_buffer,
                         int buffer_size) {
  ...
  if (!stop_after_cmd) WCND_LOGD("[Connected to Netd]\n");
...
static int send_cmd_for_netd(char *cmd, int wait_resp, char *resp_buffer,
                             int buffer_size) {
  if (!cmd) return -1;
  ...
  if (wait_resp) wait_for_netd(client_fd, 1, resp_buffer, buffer_size);
...
static int check_response_from_netd(char *resp) {
  ...
  if (!ptr_space) {
    // cannot find the ' ', there is not a result code,
    // see it as correct
    return 0;
...
static int handle_softap_cmd(int client_fd, int argc, char **argv) {
  ...
  if (!strcmp(argv[0], "setchan")) {
    ...
    if (!strcmp(argv[1], "default")) {
      ...
      snprintf(cmd + offset, (sizeof(cmd)-1) - offset, "0 softap ");
      ...
      send_cmd_for_netd(cmd, 1, resp, sizeof(resp));
      ...
      for (i = 0; i < argc; i++) {
        offset = strlen(cmd);
        snprintf(cmd + offset, (sizeof(cmd)-1) - offset, "%s ", argv[i]);
      }
      ...
      snprintf(cmd, sizeof(cmd)-1, "0 softap setchan mode");
...
int wifi_runcommand(int client_fd, int argc, char **argv) {
  ...
  if (!strcmp(argv[0], "softap")) {
    ...
    if (argc > 1) {
      ...
      snprintf(buffer, sizeof(buffer)-1, "PM:%d", power_mode);
